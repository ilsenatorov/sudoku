#!/usr/bin/python3
''' Library of functions used for extracting sudoku grid from an image
and extracting the digits from the grid. Uses OpenCV2 and numpy '''
import cv2
import operator
import numpy as np
...
def predict(digit):
    ...
    res = pts.image_to_string(digit, lang="eng", config="--psm 6 --oem 1 -c tessedit_char_whitelist=123456789")
    if res:
        return int(res)
    else:
        return 0
...
def get_grid(img, no_predict=False):
